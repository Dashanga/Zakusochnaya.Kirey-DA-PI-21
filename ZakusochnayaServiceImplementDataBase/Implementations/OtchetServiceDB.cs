...
{
    public class OtchetServiceDB : IOtchetService
    {
        private ZakusochnayaDbContext context;
        public OtchetServiceDB(ZakusochnayaDbContext context)
        {
        ...
        public void SaveProductPrice(OtchetBindingModel model)
        {
            ...
            try
            {
                ...
                var products = context.Outputs.ToList();
                //создаем таблицу
                ...
                {
                    table.Cell(i + 1, 1).Range.Text = products[i].OutputName;
        ...
        public List<SkladsLoadViewModel> GetStocksLoad()
        {
            return context.Sklads
            .ToList()
            .GroupJoin(
                context.SkladElements
.Include(r => r.Element)
.ToList(),
stock => stock,
...
new SkladsLoadViewModel
{
    ...
    FullCount = stockCompList.Sum(r => r.Number),
    Elements = stockCompList.Select(r => new Tuple<string,
    int>(r.Element.ElementName, r.Number))
})
.ToList();
        }
        public void SaveStocksLoad(OtchetBindingModel model)
        {
            ...
            try
            {
                ...
                if (dict != null)
                {
                    ...
                    foreach (var elem in dict)
                    {
                        ...
                        excelcells.Value2 = elem.SkladName;
                        ...
                        excelcells.Value2 = elem.FullCount;
                        excelcells.Font.Bold = true;
                    }
                }
                //сохраняем
        ...
        public List<PokupatelZakazsModel> GetClientOrders(OtchetBindingModel model)
        {
            return context.Zakazs
            .Include(rec => rec.Pokupatel)
            .Include(rec => rec.Output)
            .Where(rec => rec.DateCreate >= model.DateFrom &&
            rec.DateCreate <= model.DateTo)
            .Select(rec => new PokupatelZakazsModel
            {
                PokupatelName = rec.Pokupatel.PokupatelFIO,
        ...
        public void SaveClientOrders(OtchetBindingModel model)
        {
            ...
            {
                cell = new PdfPCell(new Phrase(list[i].PokupatelName, fontForCells));
                table.AddCell(cell);
            ...
            table.AddCell(cell);
            cell = new PdfPCell(new Phrase(list.Sum(rec => rec.Summa).ToString(),
            fontForCellBold))
            {
                HorizontalAlignment = Element.ALIGN_RIGHT,
                Border = 0
